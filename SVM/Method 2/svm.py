...
if __name__ == '__main__':
    ...
    # 读取数据 
    xtrain = scio.loadmat('./训练和测试数据/data_train.mat')['data_train']
    ytrain = scio.loadmat('./训练和测试数据/label_train.mat')['label_train']
    xtest = scio.loadmat('./训练和测试数据/data_test.mat')['data_test']
    ...
    for s in range(-5, 10):
        sigma = pow(10, s)
        trainDataList = xtrain.tolist()
        trainLabelList = ytrain.reshape(-1,).tolist()
        valDataList = xval.tolist()
        valLabelList = yval.reshape(-1,).tolist()
        svm = SVM(trainDataList, trainLabelList, sigma, C, epsilon)
        # 开始训练
        svm.train()
        # 开始测试
        accuracy0 = svm.test(trainDataList, trainLabelList)
        accuracy1 = svm.test(valDataList, valLabelList)
        
        sigma_accuracies.append((sigma, accuracy0, accuracy1))
        
        if acc < accuracy1:
            acc = accuracy1
            best_sigma = sigma
    
    sigma_accuracies = np.array(sigma_accuracies)
    plt.figure()
    plt.plot(np.log10(sigma_accuracies[:, 0]), sigma_accuracies[:, 1], label='train')
    plt.plot(np.log10(sigma_accuracies[:, 0]), sigma_accuracies[:, 2], label='val')
    plt.xlabel('sigma(log10)')
    plt.ylabel('accuracy')
    plt.title('accuracy of different sigma')
    plt.legend()
    ...
    for C in range(50, 500, 50):
        ...
        valDataList = xval.tolist()
        valLabelList = yval.reshape(-1,).tolist()
        svm = SVM(trainDataList, trainLabelList, best_sigma, C, epsilon)
        # 开始训练
        svm.train()
        # 开始测试
        accuracy0 = svm.test(trainDataList, trainLabelList)
        ...
        C_accuracies.append((C, accuracy0, accuracy1))
    ...
    C_accuracies = np.array(C_accuracies)
    plt.figure()
    plt.plot(C_accuracies[:, 0], C_accuracies[:, 1], label='train')
    plt.plot(C_accuracies[:, 0], C_accuracies[:, 2], label='val')
    plt.xlabel('C')
    plt.ylabel('accuracy')
    ...
    # 训练集结果输出
    pred_trn = [model.predict(trainDataList[i])[0,0] for i in range(len(trainDataList))]
    pred_trn = np.array(pred_trn)
    
    # 训练集结果评价
    con_mat = confusion_matrix(ytrain, pred_trn)
    print('训练集混淆矩阵：\n', con_mat)
    ...
    # 验证集结果输出
    pred_val = [model.predict(valDataList[i])[0,0] for i in range(len(valDataList))]
    pred_val = np.array(pred_val)
    
    # 验证集结果评价
    con_mat = confusion_matrix(yval, pred_val)
    print('训练集混淆矩阵：\n', con_mat)
    tp, fp, tn, fn = con_mat[0,0], con_mat[1,0], con_mat[1,1], con_mat[0,1]
    tpr0 = tp / (tp + fn)
    fpr0 = fp / (fp + tn)
    ...
    # 测试集结果输出
    pred_test = [model.predict(xtest.tolist()[i])[0,0] for i in range(xtest.shape[0])]
